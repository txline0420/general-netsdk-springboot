...
import com.netsdk.lib.NetSDKLib.ALARM_ENGINE_FAILURE_STATUS_INFO;
...
public class CarSenseUnit {
		...
		public static fAlarmDataCB getCallBack() {
	  	...
	  	public boolean invoke(int lCommand, LLong lLoginID, Pointer pStuEvent, int dwBufLen, String strDeviceIP,  NativeLong nDevicePort, Pointer dwUser){
	  		switch (lCommand)
	  		{
				 ...
				 *							车辆设备事件							*
				*************************************************************/
				case NetSDKLib.NET_ALARM_ENGINE_FAILURE_STATUS : //发动机故障状态上报
				{
					ALARM_ENGINE_FAILURE_STATUS_INFO msg = new ALARM_ENGINE_FAILURE_STATUS_INFO();
					ToolKits.GetPointerData(pStuEvent, msg);
					System.out.printf("【人群密度检测事件】 UTC:%s 通道号:%d 事件动作(0  未知,1 Not active,2 Active,3 Blink,4 Not Available):%d ",
							msg.UTC, msg.nChannelID, msg.emStatus);
					break;
				}
		  		default:
		  			break;
	...
	public static void main(String[]args) {
		CarSenseUnit demo = new CarSenseUnit();
		demo.BeginTest();
		demo.FinishTest();
	}
}
