...
public class PlayBox_win {
	...
	public void uploadFile(){   	
		...
		inUploadFile.pszFileDst = ToolKits.GetGBKStringToPointer("捕获.JPG");     			  // 目标文件路径,文件格式不固定
    	inUploadFile.pszFolderDst = ToolKits.GetGBKStringToPointer("/Download/");           // 目标文件夹路径,null时，为默认路径, ""或null指的是 /mnt/sdcard/;   /Download 或者  /Download/ 指的是/mnt/sdcard/Download/		
    	inUploadFile.pszFileSrc = ToolKits.GetGBKStringToPointer("d:/捕获.JPG");  			  //s 源文件路径
	...
	public void listRemoteFile() {	
		String szPath = "Download/";	  // 查询的文件路径，Download/指的是  /mnt/sdcard/Download/
		...
		if(remoteFile != null) {
			...
			for(int i = 0; i < stOut.nRetFileCount; i++) {
				try {
					System.out.println("szPath : " + new String(remoteFile[i].szPath, "GBK").trim());
	...
	public void removeFiles() {
		String[] szPath = {"Download/1.JPG", "Download/553844944532.mp4"};    // Download/4.JPG 指的是/mnt/sdcard/Download/4.JPG

		if(ToolKits.RemoveRemoteFilesArr(loginHandle, szPath)) {
    ...
    public void addOneProgramToPlayBox() {
    	...
    	// 节目名称
    	try {
			System.arraycopy("大华测试".getBytes("GBK"), 0, stIn.stuPlayBoxProgram.szProgrammeName, 0, "大华测试".getBytes("GBK").length);
		} catch (UnsupportedEncodingException e) {
			e.printStackTrace();
		}
    	...
    	switch (stIn.stuPlayBoxProgram.emProgramType) {
			case 0:   // LOGO节目
			{
				...
				String logoPath = "";
				try {
					System.arraycopy(logoPath.getBytes("GBK"), 0, stIn.stuPlayBoxProgram.stuLogoInfo.szLogoPath, 0, logoPath.getBytes("GBK").length);
				} catch (UnsupportedEncodingException e) {
					e.printStackTrace();
				}
			...
			case 1:	  // 广告条节目，对应平台的 通知管理
			{
				...
				String fontType = "黑体";
				try {
					System.arraycopy(fontType.getBytes("GBK"), 0, stIn.stuPlayBoxProgram.stuBarInfo.szFontStyle, 0, fontType.getBytes("GBK").length);
				} catch (UnsupportedEncodingException e) {
					e.printStackTrace();
				}
...
//		elementCommonInfo[1].emElementsType = EM_ELEMENTS_TYPE.EM_ELEMENTS_VIDEO;       
//		elementCommonInfo[2].emElementsType = EM_ELEMENTS_TYPE.EM_ELEMENTS_VIDEO;          
//		elementCommonInfo[3].emElementsType = EM_ELEMENTS_TYPE.EM_ELEMENTS_PLACEHOLDER;   
		...
		stuWindowsInfo.emTourPeriodType = EM_TOURPERIOD_TYPE.EM_TOURPERIOD_PROGRAMME;   
	
//		// 自定义轮训时间，单位秒, 轮训类型 emTourPeriodType 为自定义轮训时有效
//		stuWindowsInfo.nTourPeriodTime = 1 * 60;    
		...
		offSet = setPlayBoxElementInfo(EM_ELEMENTS_TYPE.EM_ELEMENTS_VIDEO, stuWindowsInfo.pstElementsBuf, offSet + size, "http://10.172.12.40:9050/20180124/5c4f325a6774485ca3c235ba76b4b3aa/186755453526183.pdf ");     // 设置具体的素材信息，转为pstElementsBuf, offSet为返回的偏移量
		
//		ToolKits.SetStructDataToPointer(elementCommonInfo[1], stuWindowsInfo.pstElementsBuf, offSet);          				  // 窗口元素通有信息转为pstElementsBuf
//		offSet = setPlayBoxElementInfo(EM_ELEMENTS_TYPE.EM_ELEMENTS_VIDEO, stuWindowsInfo.pstElementsBuf, offSet +size, "http://10.172.12.40:9050/20171214/642964c70f8942428cd69c027c459179/553844944530.mp4");    // 设置具体的素材信息，转为pstElementsBuf, offSet为返回的偏移量
//	
...
//		offSet = setPlayBoxElementInfo(EM_ELEMENTS_TYPE.EM_ELEMENTS_PLACEHOLDER, stuWindowsInfo.pstElementsBuf, offSet + size, "");    // 设置具体的素材信息，转为pstElementsBuf, offSet为返回的偏移量
    ...
    public int setPlayBoxElementInfo(int emElementsType, Pointer pstElementsBuf, int offSet, String path) {
    	...
    	switch(emElementsType) {
	    	case EM_ELEMENTS_TYPE.EM_ELEMENTS_VIDEO: {  // 视频元素		
	    		...
	    		String name = "视频素材";    
	    		try {
	    			System.arraycopy(name.getBytes("GBK"), 0, msg.szName, 0, name.getBytes("GBK").length);
	    		} catch (UnsupportedEncodingException e) {
	    			e.printStackTrace();
	    		}
	    		 ...
	    		 */
	    		try {
					System.arraycopy(path.getBytes("GBK"), 0, msg.szPath, 0, path.getBytes("GBK").length);
				} catch (UnsupportedEncodingException e) {
					e.printStackTrace();
				}
	    	...
	    	case EM_ELEMENTS_TYPE.EM_ELEMENTS_TEXT: {    // 文本元素
		    	...
		    	String fontType = "黑体";
		    	try {
					System.arraycopy(fontType.getBytes("GBK"), 0, msg.stuElementsText.szFontStyle, 0, fontType.getBytes("GBK").length);
				} catch (UnsupportedEncodingException e) {
					e.printStackTrace();
				}
		    	...
		    	try {
					System.arraycopy(content.getBytes("GBK"), 0, msg.stuElementsText.szContent, 0, content.getBytes("GBK").length);
				} catch (UnsupportedEncodingException e) {
					e.printStackTrace();
				}
    ...
    public void modifyProgramOnPlayBoxById() {
    	...
    	// 节目ID
    	String programmeID = "119809594";    			
    	try {
			System.arraycopy(programmeID.getBytes("GBK"), 0, stIn.stuPlayBoxProgram.szProgrammeID, 0, programmeID.getBytes("GBK").length);
		} catch (UnsupportedEncodingException e) {
			e.printStackTrace();
		}
    	...
    	try {
			System.arraycopy(programmeName.getBytes("GBK"), 0, stIn.stuPlayBoxProgram.szProgrammeName, 0, programmeName.getBytes("GBK").length);
		} catch (UnsupportedEncodingException e) {
			e.printStackTrace();
		}
    	...
    	switch (stIn.stuPlayBoxProgram.emProgramType) {
			...
			case 1:	  // 广告条节目，对应平台的 通知管理
			{
				...
				String content = "大华广告";
				try {
					System.arraycopy(content.getBytes("GBK"), 0, stIn.stuPlayBoxProgram.stuBarInfo.szContent, 0, content.getBytes("GBK").length);
				} catch (UnsupportedEncodingException e) {
					e.printStackTrace();
				}
    ...
    public void getAllProgramOnPlayBox() {
    	...
    	if(playboxArr != null) {	
     		...
     		for(int i = 0; i < stOut.nRetProgramCount; i++) {
     			try {
					System.out.println("\n节目名称 : " + new String(playboxArr[i].szProgrammeName, "GBK").trim());
				} catch (UnsupportedEncodingException e) {
					e.printStackTrace();
				}
				...
				case EM_PLAYBOXPROGRAM_TYPE.EM_PROGRAM_ON_PLAYBOX_LOGO:  // LOGO节目	
		 			try {
						System.out.println("LOGO路径 : " + new String(playboxArr[i].stuLogoInfo.szLogoPath, "GBK").trim());
					} catch (UnsupportedEncodingException e) {
						e.printStackTrace();
					}
				...
				case EM_PLAYBOXPROGRAM_TYPE.EM_PROGRAM_ON_PLAYBOX_BAR:  // 广告条节目
					try {
						System.out.println("广告内容：" + new String(playboxArr[i].stuBarInfo.szContent, "GBK").trim());
					} catch (UnsupportedEncodingException e) {
						e.printStackTrace();
					}
					...
					try {
						System.out.println("字体类型：" + new String(playboxArr[i].stuBarInfo.szFontStyle, "GBK").trim());
					} catch (UnsupportedEncodingException e) {
						e.printStackTrace();
					}
    ...
    public int printElementInfo(int emElementsType, Pointer pstElementsBuf, int offSet) {
		...
		case EM_ELEMENTS_TYPE.EM_ELEMENTS_VIDEO:   { // 视频元素
			...
			// 打印输出Pointer的值	
			try {
				System.out.println("素材自定义名称 : " + new String(msg.szName, "GBK").trim());
			} catch (UnsupportedEncodingException e) {
				e.printStackTrace();
			}
			...
			// 文件地址
			try {
				System.out.println("文件路径 : " + new String(msg.szPath, "GBK").trim());
			} catch (UnsupportedEncodingException e) {
				e.printStackTrace();
			}  
		...
		case EM_ELEMENTS_TYPE.EM_ELEMENTS_PICTURE:   { // 图片元素
			...
			System.out.println("是否垫片 : " + msg.bFillerState); 
			try {
				System.out.println("图片地址 : " + new String(msg.szPath, "GBK").trim());
			} catch (UnsupportedEncodingException e) {
				e.printStackTrace();
			}  
			System.out.println("播放次数 : " + msg.nPlayCount);  
		...
		case EM_ELEMENTS_TYPE.EM_ELEMENTS_TEXT:   { // 文本元素
			...
			System.out.println("是否垫片 : " + msg.bFillerState);  
			try {
				System.out.println("文本内容 : " + new String(msg.stuElementsText.szContent, "GBK").trim());
			} catch (UnsupportedEncodingException e) {
				e.printStackTrace();
			}  
			...
			try {
				System.out.println("字体类型 : " + new String(msg.stuElementsText.szFontStyle, "GBK").trim());
			} catch (UnsupportedEncodingException e) {
				e.printStackTrace();
			} 
    ...
    public void getOneProgramByIdOnPlayBox() {
    	...
    	if(bRet) {
 			try {
				System.out.println("节目名称 : " + new String(stOut.stuPlayBoxProgram.szProgrammeName, "GBK").trim());
			} catch (UnsupportedEncodingException e) {
				e.printStackTrace();
			}
			...
			case EM_PLAYBOXPROGRAM_TYPE.EM_PROGRAM_ON_PLAYBOX_LOGO:  // LOGO节目	
	 			try {
					System.out.println("LOGO路径 : " + new String(stOut.stuPlayBoxProgram.stuLogoInfo.szLogoPath, "GBK").trim());
				} catch (UnsupportedEncodingException e) {
			...
			case EM_PLAYBOXPROGRAM_TYPE.EM_PROGRAM_ON_PLAYBOX_BAR:  // 广告条节目
				try {
					System.out.println("广告内容：" + new String(stOut.stuPlayBoxProgram.stuBarInfo.szContent, "GBK").trim());
				} catch (UnsupportedEncodingException e) {
					e.printStackTrace();
				}
				...
				try {
					System.out.println("字体类型：" + new String(stOut.stuPlayBoxProgram.stuBarInfo.szFontStyle, "GBK").trim());
				} catch (UnsupportedEncodingException e) {
					e.printStackTrace();
				}
    ...
    public void delMultiProgrammesById() {
    	String[] szProGrammeIdList = {"1761321863", "979256274"};   
    ...
    public void addOneImmediProgrammePlan() {
    	...
    	try {
			System.arraycopy(planName.getBytes("GBK"), 0, stIn.stuImmePlan.szPlanName, 0, planName.getBytes("GBK").length);
		} catch (UnsupportedEncodingException e) {
			e.printStackTrace();
		}
    	...
    	String programmeName = "大华测试"; 
    	try {
			System.arraycopy(programmeName.getBytes("GBK"), 0, stIn.stuImmePlan.szProgrammeName, 0, programmeName.getBytes("GBK").length);
		} catch (UnsupportedEncodingException e) {
			e.printStackTrace();
		}
    	
    	// 节目id
    	String programmeId = "2074249508"; 
    	...
    	stIn.stuImmePlan.nPlayTime = 20;  // 播放时长, 单位 : 分钟
    ...
    public void addOneTimerProgrammePlan() {
    	...
    	try {
			System.arraycopy(planName.getBytes("GBK"), 0, stIn.stuTimerPlan.szPlanName, 0, planName.getBytes("GBK").length);
		} catch (UnsupportedEncodingException e) {
			e.printStackTrace();
		}
...
//		stIn.stuTimerPlan.nPlayDates[0] = 1;
		...
		stIn.stuTimerPlan.stuSatrtDate.dwMonth = 3;
		stIn.stuTimerPlan.stuSatrtDate.dwDay = 2;
    	
    	// 节目结束时间，时分秒
		stIn.stuTimerPlan.stuEndDate.dwYear = 2018;
		stIn.stuTimerPlan.stuEndDate.dwMonth = 3;
		stIn.stuTimerPlan.stuEndDate.dwDay = 8;
    	
		stIn.stuTimerPlan.nProgrammes = 1;  // 节目个数，可添加好几个节目，自己设置
    	
    	String programmeName= "大华测试";  // 节目名称
    	try {
			System.arraycopy(programmeName.getBytes("GBK"), 0, stIn.stuTimerPlan.stuProgrammes[0].szProgrammeName, 0, programmeName.getBytes("GBK").length);
		} catch (UnsupportedEncodingException e) {
			e.printStackTrace();
		}
    	
    	String programmeId= "208204080";   // 节目id
    	System.arraycopy(programmeId.getBytes(), 0, stIn.stuTimerPlan.stuProgrammes[0].szProgrammeID, 0, programmeId.getBytes().length);
    	
    	// 节目开始时间, 时分秒
    	stIn.stuTimerPlan.stuProgrammes[0].stuSatrtTime.dwHour = 8;
...
//		stIn.stuTimerPlan.nPlayDates[0] = 1;
		...
		stIn.stuTimerPlan.stuSatrtDate.dwDay = 25;
    	
    	// 节目结束时间，时分秒
		stIn.stuTimerPlan.stuEndDate.dwYear = 2018;
		stIn.stuTimerPlan.stuEndDate.dwMonth = 3;
		stIn.stuTimerPlan.stuEndDate.dwDay = 2;
    	...
    	String programmeId= "1298977735";   // 节目id
    	System.arraycopy(programmeId.getBytes(), 0, stIn.stuTimerPlan.stuProgrammes[0].szProgrammeID, 0, programmeId.getBytes().length);
    ...
    public void delMultiProgrammePlans() {
    	String[] szPlanID = {"863474775", "575117846"};  
    ...
    public void getAllBrieflyProgrammes() {
    	...
    	if(bRet) {
    		...
    		for(int i = 0; i < stOut.nRetCnt; i++) {
    			try {
					System.out.println("节目名称 :" + new String(stOut.stuBriProgrammes[i].szProgrammeName,"GBK").trim());
				} catch (UnsupportedEncodingException e) {
					e.printStackTrace();
				}
    ...
    public void getAllProgrammePlans() { 	
    	...
    	if(planInfo != null) {
    		...
    		for(int i = 0; i < stOut.nRetImmCnt; i++) {
    			System.out.println("\n即时计划查询");
           		try {
					System.out.println("即时发布的节目名称 : " + new String(planInfo.szImmePlan[i].szProgrammeName, "GBK").trim());
				} catch (UnsupportedEncodingException e) {
					e.printStackTrace();
				}
           		System.out.println("即时发布的节目ID : " + new String(planInfo.szImmePlan[i].szProgrammeID).trim());
        		try {
					System.out.println("节目计划名称 : " + new String(planInfo.szImmePlan[i].szPlanName, "GBK").trim());
				} catch (UnsupportedEncodingException e) {
					e.printStackTrace();
				}
    		...
    		for(int i = 0; i < stOut.nRetTimerCnt; i++) {
    			System.out.println("\n定时计划查询");
        		try {
					System.out.println("节目计划名称 : " + new String(planInfo.szTimerPlan[i].szPlanName, "GBK").trim());
        		...
        		System.out.println("节目结束日期 : " + planInfo.szTimerPlan[i].stuEndDate.toString()); 
           		...
           		for(int j = 0; j < planInfo.szTimerPlan[i].nProgrammes; j++) {
           	  		try {
						System.out.println("节目名称 : " + new String(planInfo.szTimerPlan[i].stuProgrammes[j].szProgrammeName, "GBK").trim());
					} catch (UnsupportedEncodingException e) {
						e.printStackTrace();
					}
...
//    	stOut.emPlanType = EM_PROGRAMMEPLAN_TYPE.EM_PROGRAMMEPLAN_TIMER;      // 定时节目计划信息, NET_TIMER_PLAN_INFO  stuTimerPlan有效
	...
	////////////////////////////////////////////////////////////////
	String m_strIp 			= "172.9.128.230";
	int    m_nPort 			= 37777;
	String m_strUser 		= "admin";
	String m_strPassword 	= "aaaaa111";
	////////////////////////////////////////////////////////////////
	...
	public void RunTest()
	{
		...
		menu.addItem(new CaseMenu.Item(this , "获取所有节目的简要信息", "getAllBrieflyProgrammes")); 
