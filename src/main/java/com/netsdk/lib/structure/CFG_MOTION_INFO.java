...
public class CFG_MOTION_INFO extends NetSDKLib.SdkStructure {
  ...
  /** 一代检测区域，最多32*32块区域 */
  public byte[] byRegion = new byte[MAX_MOTION_ROW * MAX_MOTION_COL];
  /** 报警联动 */
  ...
  public NetSDKLib.CFG_TIME_SECTION[] stuTimeSection =
      (NetSDKLib.CFG_TIME_SECTION[])
          new NetSDKLib.CFG_TIME_SECTION().toArray(WEEK_DAY_NUM * MAX_REC_TSECT);
  ...
  /** 视频窗口数组 */
  @Deprecated
  public CFG_MOTION_WINDOW[] stuWindows =
      (CFG_MOTION_WINDOW[]) new CFG_MOTION_WINDOW().toArray(MAX_MOTION_WINDOW);
  ...
  public CFG_DETECT_REGION[] stuRegion =
      (CFG_DETECT_REGION[]) new CFG_DETECT_REGION().toArray(MAX_MOTION_WINDOW);
  /** 前端动态检测联动 */
