...
public class NET_EVENT_INFO_EXTEND extends NetSDKLib.SdkStructure {
	/**
	 * RealUTC 是否有效，bRealUTC 为 TRUE 时，用 stuRealUTC，否则 stuRealUTC 字段无效(用原事件结构体中的
	 * 事件发生时间/事件触发时间(UTC) 字段)
	 */
	public int bRealUTC;
	/**
	 * 仅用于字节对齐
	...
	/**
	 * 事件发生的时间(标准UTC时间(不带时区夏令时偏差)), 由于事件的UTC时间在产品线之间使用的差异性, 故增加RealUTC作为标准UTC时间,
	 * 平台在收到事件解析首优先级是RealUTC, 其次是UTC.
	 */
